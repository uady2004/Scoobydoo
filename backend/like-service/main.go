...
import (
	...
	"github.com/tiktok-clone/like-service/internal/handlers"
...
func main() {
	...
	// ── Wiring ────────────────────────────────────────────────────────────────
	repo := repositories.NewLikeRepository(pool)
	svc := services.NewLikeService(repo, rdb, kafkaWriter, cfg, logger)
	handler := handlers.NewLikeHandler(svc, logger)
	...
	router.GET("/healthz", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "like"})
	})
	...
	go func() {
		logger.Info("like-service starting", zap.String("addr", srv.Addr))
	...
	logger.Info("like-service stopped")
}
