...
import (
	...
	"github.com/tiktok-clone/wallet-service/internal/handlers"
...
func main() {
	...
	// ---------- Repository ----------
	repo := repositories.NewWalletRepository(pool, logger)

	// ---------- Service ----------
	svc := services.NewWalletService(repo, cfg, logger)
	...
	// Register routes.
	h := handlers.NewWalletHandler(svc, cfg, logger)
	h.RegisterRoutes(e)
	...
	go func() {
		logger.Info("wallet-service listening", zap.String("addr", cfg.Server.Addr()))
